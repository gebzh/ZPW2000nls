function [ f0_hat, f1_hat, alpha_hat, cos_correlation, n_all_ZPW, p_all, q_all, ...
    Amplis_all,cos_correlation_all] = NLSM_Based_Algorithm_plus( x_bold, fs )
...
Timer = Timer + toc;
fprintf('time consuming NLSM %fs\n',Timer);
...
%% ========================* functions *==========================
    ...
    function [J, lph_hat, c_c] = CostCalcForCoarseEst(pp,qq)
        ...
        if ( Apls_hat(2)<Athreshold || Apls_hat(3)<Athreshold...
                || Apls_hat(4)<Athreshold || (qq <= 9 ...
    ...
    function J = CostCalcForGridSearch(pp,qq,ii,jj)
        eta = zeros(10,1);
        temp = XM(k0-k2+(ii-2*jj)+1);
        eta(1) = real(temp); eta(6) = -imag(temp);
        temp = XM(k0-k1+(ii-jj)+1);
