function [ f0_hat, f1_hat, alpha_hat, cos_correlation, n_all_ZPW, p_all, q_all, ...
    Amplis_all,cos_correlation_all] = NLSM_Based_Algorithm_1( x_bold, fs )
...
%========================* grid search *==========================
if f0c ~= -1 && f1c ~= -1
    Deltaomega = Grid_Size_Calc();
    L_h = round(0.6*pi/(fs*Deltaomega));
    omg0c = 2*pi*f0c/fs; omg1c = 2*pi*f1c/fs;
    
    Jmax = 0;
    for i = -L_h : L_h
        omg0g = omg0c + i*Deltaomega;
        for j = -L_h : L_h
            omg1g = omg1c + j*Deltaomega;
            J = CostCalcAccordingToFreqs(omg0g,omg1g);
            if J>Jmax
                Jmax = J;
                omega0g = omg0g;
                omega1g = omg1g;
...
Timer = Timer + toc;
fprintf('running time of alg1 %fs\n',Timer);
...
%% ========================* functions *==========================
    ...
    function delta_omega = Grid_Size_Calc()
        ...
        gr = 1.15;  
        delta_omega = sqrt(12*(gr-1)/gr)/N/5;
    end
